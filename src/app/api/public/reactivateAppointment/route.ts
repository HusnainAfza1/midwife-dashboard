...
// Reactivate appointment (PUT method)
export async function PUT(request: NextRequest) {
    try {
        ...
        // Validate required fields - ALL services need these
        if (!appointmentId || !updatedDate || !updatedStartTime || !updatedEndTime) {
            return NextResponse.json({
                error: "Missing required fields",
                details: "appointmentId, updatedDate, updatedStartTime, and updatedEndTime are required"
            }, { status: 400 })
        }
        ...
        if (serviceCode === "A1/A2") {
            ...
            return NextResponse.json({
                message: "A1/A2 appointment reactivated successfully",
                success: true,
                appointment: {
        ...
        if (["B1", "B2", "E1"].includes(serviceCode)) {
            ...
            if (serviceCode === "E1") {
                ...
                if (e1ClassRecord) {
                    ...
                    if (targetClass) {
                        // Remove from cancelExceptional array
                        if (targetClass.cancelExceptional && targetClass.cancelExceptional.includes(classNo)) {
                            targetClass.cancelExceptional = targetClass.cancelExceptional.filter(
                                (cn: string) => cn !== classNo
                            );
                        }

                        // Add/Update in exceptional array
                        ...
                        // Reactivate and update all clients' E1 appointments for this class
                            ...
                            if (otherPreBirthRecord && otherPreBirthRecord.appointments.E1) {
                                ...
                                for (let i = 0; i < e1Appointments.length; i++) {
                                    if (e1Appointments[i].classNo === classNo) {
                                        e1Appointments[i].status = "active";
                        ...
                        return NextResponse.json({
                            message: `E1 appointment reactivated successfully for all clients in class ${classNo}`,
                            success: true,
                            classNo: classNo,
                            totalClientsAffected: clients.length,
                            updatedDate: updatedDate,
                            updatedTime: `${updatedStartTime}-${updatedEndTime}`
                        })
                    }
                }
            } else {
                // For B1, B2 - reactivate with date/time update
                preBirthRecord.appointments[serviceCode][appointmentIndex].status = "active"
                ...
                return NextResponse.json({
                    message: `${serviceCode} appointment reactivated successfully`,
                    success: true,
                    appointment: {
            ...
            if (serviceCode === "F1") {
                ...
                if (f1ClassRecord) {
                    ...
                    if (targetClass) {
                        ...
                        // Reactivate and update all clients' F1 appointments for this class
                        const clients = targetClass.clients || [];
                            ...
                            if (otherPostBirthRecord && otherPostBirthRecord.appointments.F1) {
                                ...
                                for (let i = 0; i < f1Appointments.length; i++) {
                                    if (f1Appointments[i].classNo === classNo) {
                                        f1Appointments[i].status = "active";
                        ...
                        return NextResponse.json({
                            message: `F1 appointment reactivated successfully for all clients in class ${classNo}`,
                            success: true,
                            classNo: classNo,
                            totalClientsAffected: clients.length,
                            updatedDate: updatedDate,
                ...
                // For C1, C2, D1, D2 - reactivate with date/time update
                postBirthRecord.appointments[serviceCode][appointmentIndex].status = "active"
